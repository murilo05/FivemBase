				...
				if openIdentity[user_id] then
					...
					if identity then
						...
						if numberIdentity == user_id then
							...
							TriggerClientEvent("vRP:Identity",source,{ mode = "identity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], veiculos = identity["garage"], gemas = vRP.userGemstone(identity["discord"]), porte = identity["port"], premium = premium, sangue = bloodTypes(identity["blood"]) })
						else
							TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], porte = identity["port"], sangue = bloodTypes(identity["blood"]) })
						end
					end
